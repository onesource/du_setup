...
configure_aide() {
    ...
    # Restore the ownership and modes declared by the distribution package.
    # Ubuntu 24.04 runs AIDE as _aide; forcing this directory to root:root
    # mode 0700 prevents aideinit and dailyaidecheck from writing their files.
    if [[ -r /usr/lib/tmpfiles.d/aide-common.conf ]] &&
       command -v systemd-tmpfiles >/dev/null 2>&1; then
        systemd-tmpfiles --create /usr/lib/tmpfiles.d/aide-common.conf
    elif getent passwd _aide >/dev/null 2>&1; then
        install -d -m 0750 -o _aide -g root /var/lib/aide
    else
        install -d -m 0700 -o root -g root /var/lib/aide
    fi

    # A .new file can be left behind by a failed initialization; only the
    # active, non-empty database proves that initialization completed.
    if [[ ! -s /var/lib/aide/aide.db ]]; then
        print_info "Initializing AIDE database..."
        if command -v aideinit >/dev/null 2>&1; then
            if ! aideinit --yes --force; then
                print_error "aideinit failed. Review /var/log/aide/aideinit.errors."
                return 1
            fi
            print_info "AIDE database initialized via aideinit."
        elif command -v update-aide.conf >/dev/null 2>&1; then
            local generated_config=/var/lib/aide/aide.conf.autogenerated
            print_info "aideinit is unavailable; generating the Debian/Ubuntu runtime configuration..."
            if ! update-aide.conf ||
               ! aide --config "$generated_config" --init; then
                print_error "AIDE database initialization with $generated_config failed."
                return 1
            fi
            ...
            print_error "Neither aideinit nor update-aide.conf is available."
            return 1
        fi
        if [[ ! -s /var/lib/aide/aide.db ]]; then
            print_error "AIDE initialization did not produce /var/lib/aide/aide.db."
            return 1
        fi
    ...
    # The scheduled checker runs as _aide and must be able to read the active
    # database. aideinit can leave a newly promoted database owned by root.
    if getent passwd _aide >/dev/null 2>&1; then
    ...
    # Prefer the distribution-provided service, which runs with the package's
    # generated configuration and _aide account. Use cron only as a fallback.
    if systemctl cat dailyaidecheck.timer >/dev/null 2>&1; then
        systemctl enable --now dailyaidecheck.timer
        print_info "Enabled the distribution-provided dailyaidecheck.timer."
    elif [ ! -f /etc/cron.daily/aide ]; then
...
# This should be a verified email in your AWS SES account.
    ...
    # Debian/Ubuntu requires its generated runtime configuration.
    GENERATED_CONFIG=/var/lib/aide/aide.conf.autogenerated
    [ -x /usr/sbin/update-aide.conf ] && /usr/sbin/update-aide.conf >/dev/null
    AIDE_OUTPUT="$(/usr/bin/aide --config "$GENERATED_CONFIG" --check 2>&1)"
HOST=$(hostname)
...
exit 0
EOF
    if ! bash -n /usr/local/bin/security-log-analyzer.sh; then
        print_error "Generated security log analyzer failed syntax validation."
        return 1
    fi
...
configure_anomaly_detection() {
    ...
    write_anomaly_detector /usr/local/bin/anomaly-detector.sh
    if ! bash -n /usr/local/bin/anomaly-detector.sh; then
        print_error "Generated anomaly detector failed syntax validation."
        return 1
    fi
...
test_intrusion_detection() {
    ...
    if ! is_installed wazuh-manager; then
        print_warning "Wazuh manager is not installed; skipping its self-test."
    ...
    # AIDE
    if is_installed aide; then
        local aide_status aide_started aide_elapsed aide_config=/var/lib/aide/aide.conf.autogenerated
        print_info "Preparing the AIDE runtime configuration..."
        if command -v update-aide.conf >/dev/null 2>&1; then
            update-aide.conf >/dev/null || print_warning "update-aide.conf failed; trying the existing AIDE configuration."
        fi
        if [[ ! -r "$aide_config" && -r /etc/aide/aide.conf ]]; then
            aide_config=/etc/aide/aide.conf
        fi
        if [[ ! -r "$aide_config" ]]; then
            print_error "AIDE runtime configuration is missing; run update-aide.conf."
            aide_status=18
        else
            print_info "Running the full AIDE filesystem check. This can take several minutes; please wait..."
            aide_started=$SECONDS
            if /usr/bin/aide --config "$aide_config" --check >/dev/null 2>&1; then
                aide_status=0
            else
                aide_status=$?
            fi
            aide_elapsed=$((SECONDS - aide_started))
            print_info "AIDE filesystem check completed in ${aide_elapsed}s."
        fi
        if [[ "$aide_status" -eq 0 ]]; then
            print_success "AIDE check OK."
        elif [[ "$aide_status" -ge 1 && "$aide_status" -le 7 ]]; then
            print_warning "AIDE detected filesystem changes since the baseline (status $aide_status)."
        else
            print_error "AIDE check failed with status $aide_status; inspect /var/log/aide/."
        fi
    else
    ...
    if [ -f /usr/local/bin/anomaly-detector.sh ]; then
        if bash -n /usr/local/bin/anomaly-detector.sh &&
           /usr/local/bin/anomaly-detector.sh; then
            print_success "Anomaly detector executed."
        else
            print_error "Anomaly detector failed."
        fi
    fi
