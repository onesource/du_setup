...
install_nginx() {
    ...
    if command -v docker >/dev/null 2>&1 && docker ps -a --format "table {{.Names}}" | grep -q "^nginx$"; then
        ...
        if confirm "Do you want to remove the existing container and recreate it?"; then
            ...
            print_info "Keeping existing Nginx container. Skipping installation."
    ...
    printf '  1) Containerized (recommended) - Docker container with Nginx${NC}\n'
    printf '  2) Host-based - Direct installation on the system${NC}\n'
    ...
    # Offer security configuration options
    if confirm "Configure additional security features?"; then
...
configure_nginx_security() {
    ...
    printf '  1) Certificate Management${NC}\n'
    printf '  2) Security Monitoring${NC}\n'
    printf '  3) Vulnerability Scanning${NC}\n'
    printf '  4) All Security Features${NC}\n'
...
# Run as non-root user (will be set in Dockerfile)
user nginx;
...
http {
    ...
    more_clear_headers Server;             # Remove Server header completely
...
add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self'; connect-src 'self'; frame-ancestors 'self'; form-action 'self';" always;
